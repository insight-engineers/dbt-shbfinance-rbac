WITH demographic_not_null AS (
    -- Step 1: Filter cleaned demographic data
    SELECT *
    FROM `rbac-2024-shbfinance.credit_risk_mart.cleaned_demographic`
...
loan_origin_not_null AS (
    -- Step 2: Filter cleaned loan origin data
    SELECT *
    FROM `rbac-2024-shbfinance.credit_risk_mart.cleaned_loan_origin`
    WHERE contract_no != 'unknown' 
...
loan_month_aggr AS (
    -- Step 3: Aggregate loan and demographic data
    SELECT 
        lo.contract_no,
        lo.business_line,
        lo.loan_purpose,
    ...
    LEFT JOIN demographic_not_null AS de 
    ON lo.contract_no = de.contract_no
)

-- Step 4: Final aggregation with derived metrics
SELECT 
    aggr.contract_no,
    
    -- Monthly loan payment calculation
    (aggr.month_loan_amount + aggr.month_loan_interest) AS month_loan_payment,
    
    -- Debt-to-Income (DTI) ratio calculation
    ((aggr.month_loan_amount + aggr.month_loan_interest + aggr.month_insurance_cost) / aggr.customer_income) * 100 AS dti_ratio,
    ...
    CASE 
        WHEN aggr.disbursement_date IS NOT NULL THEN EXTRACT(YEAR FROM aggr.disbursement_date) 
        ELSE NULL 
    END AS disbursement_year,
    
    -- Check if disbursement is valid
    CASE 
        WHEN aggr.disbursement_date != '3000-01-01' THEN 1 
        ELSE 0 
    END AS is_disbursed,
